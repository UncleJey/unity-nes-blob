...
namespace Nescafe
{
    ...
    public class Cartridge
    {
        ...
        void ParseHeader(BinaryReader reader)
        {
            ...
            {
                UnityEngine.Debug.Log("Magic header value (" + magicNum.ToString("X4") + ") is incorrect");
                Invalid = true;
                return;
            }
		...
		#if UNITY_EDITOR
            UnityEngine.Debug.Log((16 * PrgRomBanks).ToString() + "Kb of PRG ROM");
		#endif

            // Size of CHR ROM (Or set CHR RAM if using it)
            ChrBanks = reader.ReadByte();
            if (ChrBanks == 0) {
                UnityEngine.Debug.Log("Cartridge uses CHR RAM");
                ChrBanks = 2;
                UsesChrRam = true;
            }
            else 
            {
			#if UNITY_EDITOR
                UnityEngine.Debug.Log((8 * ChrBanks).ToString() + "Kb of CHR ROM");
			#endif
                UsesChrRam = false;
            }
		...
		#if UNITY_EDITOR
            UnityEngine.Debug.Log("VRAM mirroring type: " + (VerticalVramMirroring ? "vertical" : "horizontal"));

            if (BatteryBackedMemory) 
				UnityEngine.Debug.Log("Cartridge contains battery backed persistent memory");

            if (ContainsTrainer) 
				UnityEngine.Debug.Log("Cartridge contains a 512 byte trainer");
		#endif
